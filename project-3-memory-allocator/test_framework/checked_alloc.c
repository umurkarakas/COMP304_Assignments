...
void *_checked_alloc(size_t nmemb, size_t size, enum allocator allocator)
{
    ...
    if (allocator == ALLOC_DEFAULT) {
        if (use_system_alloc)
            ...
            allocator = use_calloc ? ALLOC_KUCALLOC : ALLOC_KUMALLOC;
    }

    switch (allocator) {
    case ALLOC_KUMALLOC:   buf = kumalloc(alloc_size); break;
    case ALLOC_KUCALLOC:   buf = kucalloc(nmemb, size); break;
    case ALLOC_KUREALLOC:  buf = kurealloc(NULL, alloc_size); break;
    ...
    pr_debug("kumalloc: Allocated %p\n", buf);

    if (allocator == ALLOC_KUCALLOC || allocator == ALLOC_SYSCALLOC) {
        size_t i;
...
void *checked_realloc(void *ptr, size_t size)
{
    ...
    if (!size) {
        _checked_free(ptr, use_system_alloc ? ALLOC_SYSFREE : ALLOC_KUFREE);
        return NULL;
    }
